...
def detect_patterns(candles: Dict[str, np.ndarray]) -> List[Tuple[str, str, float]]:
    """Спрощена детекція торгових паттернів"""
    patterns = []
    c, h, l = candles["c"], candles["h"], candles["l"]
    
    # Додайте логіку виявлення паттернів тут
    # Наприклад: голова та плечі, двійне дно/вершина, тощо
...
def get_market_overview() -> Dict[str, any]:
    ...
    try:
        ...
        total_volume = sum(float(item['quoteVolume']) for item in data if 'USDT' in item.get('symbol', ''))
        avg_change = np.mean([float(item['priceChangePercent']) for item in data if 'USDT' in item.get('symbol', '')])
        
        return {
            'total_volume': total_volume,
            'avg_change': avg_change,
            'total_pairs': len([item for item in data if 'USDT' in item.get('symbol', '')])
        }
    except Exception as e:
