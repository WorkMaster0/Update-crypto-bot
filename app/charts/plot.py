...
from app.analytics.indicators import get_klines, ema, find_levels
from app.config import DEFAULT_INTERVAL, KLINES_LIMIT

def _to_datetime(ts: np.ndarray):
...
def plot_candles(symbol: str, interval: str = DEFAULT_INTERVAL, limit: int = 200, with_levels: bool = True) -> io.BytesIO:
    ...
    # EMA
    ax.plot(dt, e50, linewidth=1.2, label="EMA50", color="blue")
    ax.plot(dt, e200, linewidth=1.2, label="EMA200", color="orange")

    # Рівні підтримки/опору
    if levels:
        for s in levels["supports"]:
            ax.hlines(s, dt[0], dt[-1], linestyles="dashed", linewidth=0.8, color="green", alpha=0.7)
        for r in levels["resistances"]:
            ax.hlines(r, dt[0], dt[-1], linestyles="dashed", linewidth=0.8, color="red", alpha=0.7)
